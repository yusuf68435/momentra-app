...
async function getDefaultCalendarId(): Promise<string | null> {
  ...
  if (Platform.OS === 'ios') {
    ...
    return defaultCal?.id || calendars.find(cal => cal.allowsModifications)?.id || null;
  }
...
export async function addPlanToCalendar(
  ...
  try {
    ...
    if (!hasPermission) {
      return {
        success: false,
        error: lang === 'tr' ? 'Takvim izni gerekli' : 'Calendar permission required',
      };
    }
...
export async function getUpcomingEvents(
  ...
  try {
    ...
    const keywords = ['birthday', 'doğum günü', 'anniversary', 'yıl dönümü', 'yıldönümü', 'nişan', 'düğün', 'mezuniyet', 'graduation'];
 ...
 */
export function formatRelativeDate(date: string | Date, lang: 'tr' | 'en'): string {
  ...
  if (days < 30) {
    ...
    return lang === 'tr' ? `${weeks} hafta sonra` : `In ${weeks} week${weeks > 1 ? 's' : ''}`;
  }
  const months = Math.floor(days / 30);
  return lang === 'tr' ? `${months} ay sonra` : `In ${months} month${months > 1 ? 's' : ''}`;
}
